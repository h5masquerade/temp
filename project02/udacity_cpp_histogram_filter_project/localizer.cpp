	...
	float p_miss) 
{
	...
	float belief_hit_cell = p_hit / (hit_num*p_hit +(area-hit_num)*p_miss);
    float belief_miss_cell = p_miss / (hit_num*p_hit +(area-hit_num)*p_miss);
	...
	for (int i=0;i<height;i++){
		...
		for (int j=0;j<width;j++){
			if (grid[i][j]==color){
				row.push_back(belief_hit_cell);
			}
			else{
				row.push_back(belief_miss_cell);
			}
		}
		newGrid.push_back(row);
	}
